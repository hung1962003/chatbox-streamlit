...
class ChatBox:
    ...
    def from_dict(
        ...
        for name, chat in data["histories"].items():
            self.reset_history(name)
            for h in chat.get("history", []):
                msg = {
                    "role": h["role"],
                    "elements": [OutputElement.from_dict(y) for y in h.get("elements", [])],
                    "metadata": h.get("metadata", {}),
                }
                self.other_history(name).append(msg)
            self.other_context(name).update(chat.get("context", {}))
